...
- **Plugin marketplace updated** — 45 → 51 skills, added keywords: mutation-testing, tdd, contract-validation, quality-gates, issue-validation
...
- **`docs-create` and `docs-update` use `$JAAN_DOCS_DIR` for customizable output** ([#95](https://github.com/parhumm/jaan-to/issues/95)) — Both documentation skills now use the new `$JAAN_DOCS_DIR` path variable (default: `jaan-to/docs/`) instead of hardcoded paths. Users can customize the docs output location via `paths_docs` in settings.yaml or `JAAN_DOCS_DIR` environment variable. Path tables, `allowed-tools` permissions, staleness detection, and archive operations all updated. Skills also share template/learn files (`jaan-to:docs.template.md`, `jaan-to:docs.learn.md`) and reference STYLE.md from plugin source. Closes [#95](https://github.com/parhumm/jaan-to/issues/95)
...
- **`verify-install.sh` updated for removed bootstrap artifacts** ([#98](https://github.com/parhumm/jaan-to/issues/98)) — Removed stale checks for `STYLE.md`, `create-skill.md`, and `research/` directory that bootstrap no longer creates. Reference docs are now verified in the plugin source instead
- **Stale documentation updated to match bootstrap changes** ([#98](https://github.com/parhumm/jaan-to/issues/98)) — Updated `bootstrap.md`, `seed-files.md`, and quality-reviewer agent to reflect that STYLE.md and create-skill.md are loaded from plugin source at runtime. Agent references now use `$JAAN_*` variables and `${CLAUDE_PLUGIN_ROOT}`
