...
### New
- **Changelog rewrite skill** — Transform technical changelogs into user-friendly product updates with `/pm-changelog-rewrite`. Supports latest, specific version, or full rewrite modes.
...
### New
- **Issue reporting skill** — Report bugs and feature requests directly to any GitHub or GitLab repository with `/qa-issue-report`. Auto-detects your repo, collects environment info, and sanitizes private data before submission.
- **Mutation testing skill** — Validate how thorough your test suite really is with `/qa-test-mutate`. Supports multiple stacks (Node.js, PHP, Go, Python) and iterates until quality targets are met.
- **TDD orchestration skill** — Run full Red/Green/Refactor TDD cycles with `/qa-tdd-orchestrate`. Coordinates test writing, implementation, and refactoring phases automatically.
- **API contract validation skill** — Validate your API specs through a multi-tool pipeline with `/qa-contract-validate` — linting, breaking change detection, conformance checks, and fuzz testing.
- **Quality gate skill** — Get a composite quality score across tests, security, code review, and mutation testing with `/qa-quality-gate`. Routes decisions: auto-approve high quality, flag low quality for human review.
- **Issue validation skill** — Automatically validate reported issues against your codebase with `/qa-issue-validate`. Checks if bugs are real, finds root causes, and posts findings back to the issue. (#77)
- **Roadmap management skills** — Two new skills for any project: `/pm-roadmap-add` for prioritized roadmap planning (MoSCoW, RICE, or Value-Effort) and `/pm-roadmap-update` for reviewing, marking progress, and reprioritizing.
- **Workflow pattern discovery skill** — Detect repeated patterns in your AI sessions and get skill suggestions to automate them with `/pm-skill-discover`. (#80)
...
- **Team orchestration** — New `--track tdd` option in `/team-ship` for full TDD pipeline workflows.
...
- **Documentation fetch skill** — Fetch and cache library docs on demand with `/dev-docs-fetch`. Auto-detects your tech stack, caches results for 7 days, and works standalone or as part of other skills.
...
- **Team orchestration skill** — Assemble a virtual team of AI agents (PM, UX, Backend, Frontend, QA, DevOps, Security) to ship ideas from concept to production with `/team-ship`. Three modes: fast (8 skills), full (20 skills), or detect-only (5 auditors). (#77)
...
### New
- **Backend PR review skill** — Review backend pull requests across any stack (PHP, Node.js, Python, Go, Rust) with `/backend-pr-review`. Detects security issues, performance problems, and code quality concerns with confidence scoring. Supports GitHub and GitLab. (#110)
...
### New
- **Incremental audits** — `/detect-dev` now supports `--incremental` to scan only files changed since the last audit, making re-audits much faster. (#81)
...
- **Test execution skill** — Run tests, diagnose failures, and auto-fix common issues with `/qa-test-run`. Supports Node.js, PHP, and Go. (#82)
- **Build verification skill** — Validate your project builds and services run correctly with `/dev-verify`. Checks types, compiles, starts services, and runs smoke tests. (#78, #85)
...
### Improved
- **Smarter issue reporting** — `/jaan-issue-report` now submits directly to GitHub by default when authenticated. No local file clutter unless you want it.
...
### New
- **Output integration skill** — Copy generated outputs into your actual project with smart placement, config merging, and dependency installation using `/dev-output-integrate`. (#70)
- **Deployment activation skill** — Set up secrets, provision platforms (Railway, Vercel, Fly.io), and trigger deployment pipelines with `/devops-deploy-activate`. (#70)
...
- **5 new skills completing the spec-to-ship pipeline:**
  - **Service implementation** (`/backend-service-implement`) — Generate full service code with business logic from your specs.
  - **Test generation** (`/qa-test-generate`) — Produce runnable unit and E2E tests from your BDD test cases.
  - **Security remediation** (`/sec-audit-remediate`) — Generate targeted security fixes from audit findings with regression tests.
  - **Infrastructure scaffolding** (`/devops-infra-scaffold`) — Generate CI/CD workflows, Dockerfiles, and deployment configs.
  - **Project assembly** (`/dev-project-assemble`) — Wire backend + frontend scaffold outputs into a runnable project.
...
### New
- **Issue reporting skill** — Report bugs, feature requests, or documentation problems to the jaan-to GitHub repo with `/jaan-issue-report`. Auto-drafts from your current session context and sanitizes private data.

### Improved
- **Opt-in activation** — jaan.to is now opt-in per project. Run `/jaan-init` to activate. Projects without `jaan-to/` are completely unaffected.
...
### New
- **Flowchart generation skill** — Generate Mermaid flowcharts from PRDs, docs, or codebases with `/ux-flowchart-generate`. Supports user flows, system flows, architecture diagrams, and state flows. Renders directly in GitHub.
...
### New
- **Changelog generation skill** — Generate changelogs from git history with `/release-iterate-changelog`. Parses commit messages, suggests version bumps, analyzes user impact, and formats in Keep a Changelog style.
...
### New
- **WordPress PR review skill** — Review WordPress plugin pull requests for security, performance, and coding standards with `/wp-pr-review`. Supports GitHub and GitLab.
...
### New
- **Backend scaffold skill** — Generate production-ready backend code from specs with `/backend-scaffold`. Supports Node.js (Fastify + Prisma), PHP (Laravel/Symfony), and Go (Chi/stdlib). Includes routes, services, validation, and error handling.
- **Frontend scaffold skill** — Convert designs to React/Next.js component scaffolds with `/frontend-scaffold`. Includes TailwindCSS, typed API hooks, state management, and URL state.
...
### Important
- **Breaking change** — `/pack-detect` renamed to `/detect-pack`.
...
- **6 audit skills for comprehensive project analysis:**
  - `/detect-dev` — Engineering quality audit with scoring across 11+ language ecosystems.
  - `/detect-design` — Design system detection with drift and inconsistency findings.
  - `/detect-writing` — Writing and content system analysis with tone mapping.
  - `/detect-product` — Product feature extraction with monetization and analytics scanning.
  - `/detect-ux` — UX audit with route extraction, heuristic evaluation, and flow diagrams.
  - `/detect-pack` — Consolidate all audit results into a scored knowledge index with risk heatmap.
...
### New
- **Data model skill** — Generate comprehensive data model documentation with ER diagrams, table definitions, indexes, and migration playbooks using `/backend-data-model`.
...
### New
- **API contract skill** — Generate OpenAPI 3.1 contracts from your API resources with `/backend-api-contract`. Includes error schemas, pagination, and a companion quick-start guide.
...
### Improved
- **Cleaner skill commands** — Skill commands simplified (e.g., `/jaan-to-pm-prd-write` is now `/pm-prd-write`).
...
### New
- **Frontend design skill** — Generate distinctive, production-grade frontend components with `/frontend-design`. Creates working React/Vue/vanilla components with bold designs, full accessibility, and responsive layouts using modern CSS.
...
### New
- **Learning insights dashboard** — View accumulated learning across all skills with `/learn-report`. See coverage, common mistakes, and actionable next steps.
...
### New
- **UX research synthesis skill** — Transform raw research data (interviews, usability tests, surveys) into actionable insights with `/ux-research-synthesize`. Three modes: Speed, Standard, or Cross-Study meta-analysis.
...
### New
- **Test case generation skill** — Generate production-ready BDD/Gherkin test cases from acceptance criteria with `/qa-test-cases`. Minimum 10 tests per criterion covering positive, negative, boundary, and edge cases.
...
### New
- **Microcopy writing skill** — Generate multi-language UI text with cultural adaptation using `/ux-microcopy-write`. Supports 7 languages including Persian (with RTL), Turkish, German, French, and Russian. Covers 11 microcopy categories with tone-of-voice management.
...
### New
- **Frontend task breakdown skill** — Transform UX design handoffs into production-ready task breakdowns with `/frontend-task-breakdown`. Includes component inventories, state matrices, estimates, dependency graphs, and performance budgets.
...
### New
- **Heatmap analysis skill** — Analyze heatmap CSV exports and screenshots to generate prioritized UX reports with `/ux-heatmap-analyze`. Includes ICE-scored recommendations and cross-reference validation.
...
### New
- **Roadmap maintenance skill** — Keep your roadmap in sync with your codebase using `/roadmap-update`. Detects stale tasks, syncs status, and generates progress reports.
...
### New
- **User story skill** — Generate user stories with Given/When/Then acceptance criteria using `/pm-story-write`. INVEST validation, edge case mapping, story splitting, and Jira/Linear export.
...
### Improved
- **Unified research skill** — Topic research and file/URL indexing merged into a single `/pm-research-about` command with automatic input detection.
...
- **jaan.to launches** with 10 skills, 2 AI agents, and a learning system:
  - **PRD writing** (`/pm-prd-write`) — Generate comprehensive product requirements documents.
  - **GTM tracking** (`/data-gtm-datalayer`) — Generate Google Tag Manager dataLayer specifications.
  - **Deep research** (`/pm-research-about`) — Research any topic with source citations.
  - **Documentation** (`/docs-create`, `/docs-update`) — Create and maintain project documentation.
  - **Skill management** (`/skill-create`, `/skill-update`) — Create and update skills.
  - **Learning** (`/learn-add`) — Capture lessons that improve future skill runs.
  - **Roadmap** (`/roadmap-add`) — Add prioritized tasks to your roadmap.
