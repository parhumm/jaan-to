...
allowed-tools: Read, Glob, Grep, Task, WebSearch, Write(.claude/skills/**), Write(docs/**), Edit, Bash(git checkout:*), Bash(git branch:*), Bash(git add:*), Bash(git commit:*), Bash(git push:*), Bash(gh pr create:*)
...
- `.claude/skills/to-jaan-skill-update/LEARN.md` - Past lessons
...
Read `.claude/skills/to-jaan-skill-update/LEARN.md` if it exists:
...
Read all files in `.claude/skills/{name}/`:
...
If approved:
1. Write SKILL.md
2. Write template.md (if modified)
3. Write LEARN.md

Confirm: "Skill files updated in `.claude/skills/{name}/`"
...
```bash
git add .claude/skills/{name}/ docs/skills/{role}/{name}.md
