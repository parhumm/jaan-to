...
# Exit codes: 0 = allow (default), 2 = deny — the BLOCKED reason goes to STDERR, which is what
# Claude Code feeds back to the model on exit 2 (stdout is dropped and showed as "No stderr output").
# Stderr cap: ≤1,200 chars (~300 tokens)
...
if echo "$COMMAND" | grep -qE '\|\s*(/\S+/|(\S+/)?env\s+(\S+/)?)(bash|sh|zsh|dash|python|python3|perl|ruby|node|php)\b'; then
  echo "BLOCKED: Piping to path-based or env-wrapped interpreter is not allowed." >&2
  exit 2
fi
...
if echo "$COMMAND" | grep -qE '(^|[;&|])\s*(source|\.)\s'; then
  if [ -n "$PLUGIN_DIR" ] && ! echo "$COMMAND" | grep -qF "$PLUGIN_DIR"; then
    echo "BLOCKED: source/dot commands may only load plugin scripts." >&2
    exit 2
  fi
fi
...
if echo "$COMMAND" | grep -qE '\{(curl|wget|rm|chmod|sudo|eval|bash|sh|nc|ncat|cat|python|python3|perl|ruby|node|php|env|xargs|find|dd|mkfs|kill|pkill|tee|tar),'; then
  echo "BLOCKED: Brace expansion with dangerous command detected — potential injection." >&2
  exit 2
fi

# Block brace expansion piped to any command ({anything,...} | cmd)
if echo "$COMMAND" | grep -qE '\{[^}]+,[^}]+\}\s*\|'; then
  echo "BLOCKED: Brace expansion piped to command is not allowed." >&2
  exit 2
fi

# Block brace expansion with args piped to shell interpreter ({echo,payload} file | bash)
if echo "$COMMAND" | grep -qE '\{[^}]+,[^}]+\}.*\|\s*(bash|sh|zsh|dash|python|python3|perl|ruby|node|php)'; then
  echo "BLOCKED: Brace expansion piped to shell interpreter is not allowed." >&2
  exit 2
fi
...
if echo "$COMMAND" | grep -qE "(\w|[\"')])\s*[<>]\("; then
  echo "BLOCKED: Process substitution as command argument is not allowed." >&2
  exit 2
fi

# Block sed execute flag (weaponized sed — CVE-2025-66032 family, any delimiter).
# The flag must sit right after the closing delimiter, inside the same quoted s-command. The earlier
# pattern let the two inner wildcards run past the closing quote, so a harmless substitution with a
...
if echo "$COMMAND" | grep -qE 'sort\s.*--compress-program'; then
  echo "BLOCKED: sort --compress-program is not allowed — potential code execution." >&2
  exit 2
fi
