...
The `/ux-microcopy-write` skill generates production-ready microcopy for UI components in up to 7 languages with full cultural adaptation, tone-of-voice consistency, and RTL/LTR support.
...
```bash
/ux-microcopy-write "user authentication feature"
```

### With Feature Description

```bash
/ux-microcopy-write "Login form with email/password, forgot password link, and sign up CTA. Need error messages for invalid credentials and empty fields."
```

### Multi-Category Example

```bash
/ux-microcopy-write "E-commerce checkout flow: payment form, shipping address, order summary, empty cart state, success confirmation"
```
...
**Command**:
```bash
/ux-microcopy-write "Login form with email and password fields"
```
