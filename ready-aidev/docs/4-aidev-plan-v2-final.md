# aidev — Final Implementation Plan v2.0

**Status:** Ready to build · **Date:** 2026-08-23
**Supersedes:** `deep-research-report.md` (architecture research) and
`aidev-implementation-plan.md` v1.0 (build plan)
**Incorporates:** `aidev at R0/R1: The Achievable Frontier` (performance research)
...
- [ ] Killing the daemon mid-run and restarting resumes from checkpoint with no lost or duplicated
      side effects.
...
```text
  ...
  └──┬──────────────────────────────────────────────┘
     │
  ┌──▼──────────────────────────────────────────────┐
  │ PHASE 2 · VERIFY        (no model)              │
  │   1. apply patch → parse/compile                 │
  │   2. warm type-check on changed files            │
  │   3. impacted tests, fail-fast                   │
  ...
  └──┬──────────────────────────────────────────────┘
     ...
     ├── pass ──────────────────────────► READY
     │
     └── fail ──► PHASE 3 · REPAIR  (1 model call, ~30% of tasks)
...
- Static classifier returns ≥ R2, or lands in the ambiguous mid-band and the tie-break model call
  returns ≥ R2.
- Any deterministic risk floor fires (auth, migration, payment, infra, new dependency).
...
Deterministic scorer over: files touched · symbols touched · import fan-out · co-change coupling
strength · test blast radius · cyclomatic delta · path rules · diff size.

```text
R0  AST-equivalent or comment/string-only change, 1 file, no fan-out
R1  ≤3 files, single module, fan-out below threshold, tests exist, no floor fired
R2  cross-module, public interface, elevated fan-out, or one floor at elevated/high
R3  any floor at critical, or ≥2 dimensions at high
```

Ambiguous mid-band (between R1 and R2 by the scorer's own margin) is the **only** place a
classification model call is permitted, and it may only escalate.

Target accuracy ≥85% on the R1/R2 boundary before the model classifier is removed entirely (E-03).

### 7.4 Test impact analysis (P-09)

| Ecosystem | Mechanism | Fail-safe |
|---|---|---|
| Python | import-graph (`pytest-impacted` / `tach`-style module graph); optional coverage map | Full impacted-module suite when the map is stale |
| Node/TS | `jest --findRelatedTests` / vitest related | Same |
| Generic | commands declared in `project.toml` | Always full suite |

**Fail-safe triggers — run everything:** lockfile or manifest changed · config/build files changed ·
coverage map older than the threshold · any floor fired · class ≥ R2. The cardinal sin is skipping a
test that would have failed; bias toward over-selection.
...
### 8.1 Prefix manager (P-03, I-11)

The cached prefix is assembled once per task and is **byte-identical across every call in that
task**. Structure, in order:
...
- No timestamps, UUIDs, run ids, or counters before the breakpoint.
- Tool list is frozen at session creation; dynamic registration is forbidden (it invalidates
  everything).
- The prefix is hashed and stored; every subsequent call asserts the hash matches, and a mismatch
  is a hard error, not a warning (I-11).
...
**Claude worker.** Structured path uses JSON output with a JSON-schema constraint, reading the
structured output field. Streaming path uses stream-JSON with the verbose flag, adding partial
...
read-only sandbox for analysis roles, workspace-write for implementation, never full-access.
Approvals set to never, with the sandbox as the actual boundary. Config hygiene flags used on every
invocation. Reasoning effort set per tier (P-12). Prefer the persistent server mode for pooling.
Never emit the removed full-auto flag.

### 8.4 Untrusted-repository hardening (I-10, unchanged and non-negotiable)
...
context *data* at repository trust level. Opt-back-in is per-file in `project.toml` — an explicit
human act.
...
### 8.5 Effort and escalation (P-12, P-13)
...
### 12.1 Fast-path states

```text
PREPARE → IMPLEMENT → VERIFY → READY
              ↑           │
              └── REPAIR ─┘        (max 1)
                          └──────► ESCALATED  (→ heavy path or human)
```
...
| From | To | Guard |
|---|---|---|
| `PREPARE` | `IMPLEMENT` | closure predicate holds; class ≤ R1; worktree + base SHA recorded |
| `PREPARE` | `ESCALATED` | class ≥ R2, floor fired, or closure unsatisfiable within budget |
| `IMPLEMENT` | `VERIFY` | patch applied; diff ≤ size ceiling; risk recomputed on the diff |
...
1. Load newest checkpoint.
2. Reconcile: worktree `HEAD` must equal `checkpoint.workspace_sha`; if not, hard-reset **inside the
   aidev worktree only** and log `workspace.reconciled`.
3. Reap orphaned worker PIDs with no terminal event; mark `interrupted`; evict their sessions.
4. Resume at the checkpoint state with `attempt_no + 1`. Session cache is cold after a restart —
   record the re-write cost rather than hiding it.
5. If the worktree is gone, mark `FAILED` with `reason=workspace_lost`, keep all artifacts.
...
`static.localized|classified|closure_satisfied|closure_failed` · `tia.selected|failsafe` ·
`typecheck.daemon_started|ready` · `escalation.fastpath_exit` — alongside the v1.0 set.
...
Prefer argv lists over shell strings. Split compound commands on `&&`, `||`, `;`, `|`, `|&`, `&`,
newlines; each subcommand passes independently. Strip known wrappers (`timeout`, `time`, `nice`,
`nohup`, `stdbuf`, `command`, `builtin`, bare `xargs`) before matching. Treat environment runners
(`npx`, `docker exec`, `devbox run`, `mise exec`, `direnv exec`, `uv run`, `poetry run`) as opaque —
rules must name the inner command. Exec-capable forms (`find -exec`, `find -delete`, `watch`,
`setsid`, `flock`, `ionice`) require exact-match rules. Check redirection targets as writes.
...
predicate** with pyright/ts-morph resolvers · deterministic R-tier classifier · risk floors with
fixtures · **test impact analysis** for Python and Node with fail-safe · `localization_result`
recording.

Exit:
- [ ] Localizer produces ≤10 candidates; `in_final_diff` precision measured on the fixture corpus.
- [ ] Closure predicate is computed with **zero model calls** (I-12 audit test).
- [ ] Classifier agrees with hand-labelled tiers on ≥85% of a 100-PR historical sample (E-03).
- [ ] Each floor rule has a positive and a negative fixture test.
- [ ] TIA selects a strict subset on a fixture repo and correctly falls back to full suite on
      manifest change, stale map, and floor fire.

### M4 — Worker runtime · **L** · depends on S-01, S-02, S-03

Prefix manager with hash assertion · session pool with fork and TTL watchdog · Claude worker ·
Codex worker · capability probe · structured-output path · streaming parser · cancellation
semantics · usage and cache extraction · model registry · **untrusted-repo hardening**.

Exit:
- [ ] A fast-path task completes with **one model call** and a measured cache-read ratio > 0.8 on
      the second and subsequent calls.
- [ ] Prefix hash is identical across every call in a task; a deliberate prefix mutation raises
      `prefix_mismatch` (I-11 test).
- [ ] A fixture repo containing a malicious hook and a malicious MCP config produces **zero**
      execution — proven by a canary file that is never created (I-10).
- [ ] Cancellation: SIGINT ends the turn and records usage; SIGTERM fallback exits cleanly; no
      orphan processes (pgrep in teardown).
...
- [ ] Measured against solo-agent baseline on 20 R0 and 20 R1 tasks: report actual speed and token
      ratios. **If they miss §2.1, the cause is documented and the next milestone is a fix, not a
      feature.**
- [ ] Median fast-path model calls ≤ 2.
- [ ] Escalation to heavy path is exercised and carries context forward without re-derivation.
...
Each item individually gated by M8 evidence: OS sandbox backends (bubblewrap, rootless podman,
macOS profiles) for strict R3 · parallel DAG inside heavy-path IMPLEMENT with predicted read/write
sets · co-change model retraining automation · DuckDB analytics over exported Parquet · localizer
weight tuning from `in_final_diff` data · 1-hour TTL policy per project from measured gap p90.
...
M0 ──► M1 ──► M2 ──► M3 ──► M4 ──► M5 ──► M6 ──► M7 ──► M8 ──► M9
                     ▲              (S-01..S-03 before M4)
                     └── static core before workers: never pay for what a compiler knows
```

Do not reorder. Specifically: the **broker before real workers** (no model runs without the
boundary), the **static core before workers** (it removes calls rather than optimising them), and
...
never averaged. Never loosen a policy on a handful of successes.
...
| ID | Spike | Box | Blocks |
|---|---|---|---|
| **S-01** | Capture `--help` and recorded event streams from the *installed* Claude and Codex versions; confirm every flag and field used in §8.3; check for a persistent server/daemon mode on each; confirm whether cached reads count against rate limits per model | 1.5 d | M4 |
...
| **S-04** | Measure cache mechanics empirically: minimum prefix, actual TTL behaviour, write premium, and what a prefix mutation costs | 1 d | M4 |
...
| **S-06** | Baseline the target project's own test/lint/build/typecheck times, to size the verification budget and the TTL policy | 0.5 d | M3 |
...
| 030 | Evaluation before optimisation | D-34 | M8 |
