# aidev — Implementation Baseline v5.1 (FROZEN)

**Status:** **FROZEN v1 baseline — begin implementation.** M0–M2 ready now. M3 gated on S-05/S-06 and
the written assessment contract. M4 gated on S-01–S-04, S-07, S-08. Remaining uncertainty is resolved
by S-01…S-08 and E-01…E-06 — **not by a v6.** · **Date:** 2026-08-23
**Supersedes:** `deep-research-report.md`, `aidev-implementation-plan` v1.0, `aidev-plan-v2`,
`aidev-plan-v3-baseline`, `aidev-plan-v4`, `aidev-plan-v5`
...
### 0.1 From v5.0 → v5.1 (freeze patch)
...
### 0.2 From v4.0 → v5.0 (consistency freeze)
...
### 0.3 From v3.0 → v4.0 (baseline freeze review)
...
### 0.4 From v2.0 → v3.0 (design review)
...
### 0.5 From v1.0 → v2.0 (performance research)
...
| I-25 | **After a rollback, workspace-local effects from a superseded generation are voided and re-derived, even at `SUCCEEDED`** | Crash-after-write-before-checkpoint fixture; journal and tree agree post-recovery |
...
| **P-39** | **Workspace generation scopes local effects.** Each action declares `effect_scope`; a rollback increments the generation and **voids workspace-local attempts even at `SUCCEEDED`**, while external effects are generation-independent. | A journal that says a write happened, over a tree where it did not, is a worse failure than not recording it |
...
| `POSTDIFF` | `ESCALATED` | post-diff class ≥ R2, `force_heavy` on actual paths, `context_gap` non-empty, or size ceiling exceeded — **restore pre-IMPLEMENT checkpoint first**, patch carried as evidence only |
...
2. Load newest checkpoint; read its `workspace_generation` as the reconciliation watermark.
3. Reconcile the tree: worktree `HEAD` must equal `checkpoint.workspace_sha`. If not, hard-reset
   **inside the aidev worktree only**, `run.workspace_generation += 1`, and log
   `workspace.reconciled`.
4. **Void the superseded generation** (P-39, I-25). Every `action_attempt` whose logical action has
   `effect_scope = workspace_local` and whose `workspace_generation` is at or below the watermark is
   marked `void` — **including attempts recorded `SUCCEEDED`**. Their outcome described a tree that
   the reset erased; carrying it forward is the inconsistency v5.0 shipped:

   ```text
   checkpoint A  →  fs.write SUCCEEDED  →  crash  →  reset to A
   journal says: the write happened.   worktree says: it did not.
   ```

   A voided action is re-derived from intent on the next attempt, in the new generation. Its history
...
```sql
-- workspace generation: incremented on every hard reset/rollback of the run worktree (P-39)
...
CREATE INDEX idx_attempt_gen ON action_attempt(run_id, workspace_generation);
...
| `action_generation_voided` | workspace-local attempt survives a rollback that erased its effect (P-39, I-25) | mark `void`, re-derive from intent in the new generation; not an error | n/a |
...
environment provisioning with platform-complete `EnvKey` (§15A) · `localization_result`
...
| I-10 | Malicious hook + `.mcp.json` canary files never created | M4 |
...
| I-25 | Crash after `fs.write` SUCCEEDED, before checkpoint: recovery voids and re-derives; tree and journal agree | M2 |
...
agent_config_files = []          # opt-in is per-file AND per-approved-hash (P-22)
```
