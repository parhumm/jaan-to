# aidev — Implementation Baseline v5.0

**Status:** **FINAL v1 baseline. M0–M2 ready to build. M3 gated on S-05/S-06 and the written
assessment contract. M4 gated on S-01–S-04, S-07, S-08.** · **Date:** 2026-08-23
**Supersedes:** `deep-research-report.md`, `aidev-implementation-plan` v1.0, `aidev-plan-v2`,
`aidev-plan-v3-baseline`, `aidev-plan-v4`
...
### 0.1 From v4.0 → v5.0 (consistency freeze)
...
### 0.2 From v3.0 → v4.0 (baseline freeze review)
...
### 0.3 From v2.0 → v3.0 (design review)
...
### 0.4 From v1.0 → v2.0 (performance research)
...
4. **The boundary precedes the effect.** The Action Broker, trust root, action journal, and egress
...
| D-20 | v1 sandbox = provider-native isolation + broker policy; strict OS isolation becomes an M7 shipping prerequisite for R2/R3 tool access if S-07 fails; remaining backends in M9 |
...
CREATE TABLE action_journal (
  id TEXT PRIMARY KEY, run_id TEXT NOT NULL REFERENCES run(id),
  action_id TEXT NOT NULL,             -- durable logical identity across retry/recovery (P-35)
...
| Role | Process filesystem | Network | Secrets | Model tool surface |
|---|---|---|---|---|
| PREPARE (static) | worktree read + env store | provisioning only, Broker-gated | deny | **none — no model** |
| FAST IMPLEMENTER | provider process: neutral cwd only | provider egress only | deny | **none (P-29)** |
| PATCH APPLIER | run worktree rw | deny | deny | **none — aidev, not a model** |
| HEAVY IMPLEMENTER | run worktree rw | provider egress only | deny | `BROKERED_TOOLS`, root-scoped, or none |
| REVIEWER / ARCHITECT | read-only roots | provider egress only | deny | read-only `BROKERED_TOOLS` if S-07 proven, else none |
| VERIFIER | worktree rw + env store ro | **deny** (steady state) | synthetic only | none — aidev runs checks |
| PRODUCTION | — | — | — | denied; gate only |

Strict OS backends (bubblewrap, rootless podman, macOS profiles) normally land in M9, **but move
into M7 as a shipping prerequisite for R2/R3 tool access if S-07 fails to prove per-action
interception** (D-20).
...
| `action_unknown_external` | non-idempotent external action left `UNKNOWN` (P-23, I-15) | **halt in `BLOCKED_HUMAN`** for reconciliation; never replay | 0 |
...
| `aidev journal <run>` | Action journal: open, reconciled, and halted actions |
...
| **E-03** | Preflight and post-diff classifier accuracy at the R1/R2 boundary | Independently label ≥100 historical PRs; require ≥85% preflight before retiring the ambiguous-band tie-break, and ≥95% post-diff on a held-out set. **Schedule the labelling during M1–M2**; it is a data task, not something to discover at M3 exit | M3 |
